...
def main():
    ...
    if 'linux' in child.before.decode(encoding='utf-8').lower():
        LINUX_MODE = 1
    else:
        LINUX_MODE = 0
    ...
    duration_ = duration.decode(encoding='utf-8')
    days = '0'
    hours = '0'
    mins = '0'
    if 'day' in duration_:
        child.match = re.search(r'([0-9]+)\s+day',duration_)
        days = str(int(child.match.group(1)))
    if ':' in duration_:
        child.match = re.search('([0-9]+):([0-9]+)',duration_)
        hours = str(int(child.match.group(1)))
        mins = str(int(child.match.group(2)))
    if 'min' in duration_:
        child.match = re.search(r'([0-9]+)\s+min',duration_)
        mins = str(int(child.match.group(1)))
    print()
    print('Uptime: %s days, %s users, %s (1 min), %s (5 min), %s (15 min)' % (
        duration_, users, av1, av5, av15))
    child.expect (COMMAND_PROMPT)
